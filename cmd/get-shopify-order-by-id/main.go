...
func main() {
	if len(os.Args) < 2 {
		fmt.Println("Usage: go run cmd/get-shopify-order-by-id/main.go <shopify_order_id>")
	...
	fmt.Printf("🔍 Fetching order from Shopify by ID: %s\n\n", orderIDStr)

	// Execute query with GID variable
